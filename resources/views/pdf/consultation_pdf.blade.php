...
<body>
    ...
    <h2 class="consultation-title">CONSULTATION RECORD</h2>
    ...
    <div class="box">
        <div class="box-title">{{ $formatted['createdStr'] }}</div>
        <table class="vitals-table">
            <tr>
                <td colspan="4"><span class="label"><strong>Chief Complaint:</strong></span> {{ $val($consultation, 'chief_complaint') }}</td>
            </tr>
            <tr>
                <td><span class="label"><strong>Temp:</strong></span> {{ $val($consultation, 'temperature', ' °C') }}</td>
                <td><span class="label"><strong>BP:</strong></span> {{ $val($consultation, 'blood_pressure') }}</td>
                <td><span class="label"><strong>PR:</strong></span> {{ $val($consultation, 'pulse_rate', ' bpm') }}</td>
                <td><span class="label"><strong>RR:</strong></span> {{ $val($consultation, 'respiratory_rate', ' / min') }}</td>
            </tr>
            <tr>
                <td><span class="label"><strong>SpO₂:</strong></span> {{ $val($consultation, 'spo2', ' %') }}</td>
                <td><span class="label"><strong>Pain Scale:</strong></span> {{ $val($consultation, 'pain_scale') }}</td>
                <td><span class="label"><strong>LMP:</strong></span> {{ $formatted['lmpStr'] ?? '—' }}</td>
                <td><span class="label"><strong>BP (repeat):</strong></span> {{ $val($consultation, 'blood_pressure') }}</td>
            </tr>
            <tr>
                <td colspan="2"><span class="label"><strong>Assessment:</strong></span> {{ $val($consultation, 'assessment') }}</td>
                <td colspan="2"><span class="label"><strong>Intervention:</strong></span> {{ $val($consultation, 'intervention') }}</td>
            </tr>
            <tr>
                <td colspan="4"><span class="label"><strong>Medicine Given:</strong></span>
                    @php
                    $dispensedMedicines = $consultation->getDispensedMedicinesFromTransactions() ?? [];
            ...
            <tr>
                <td colspan="4"><span class="label"><strong>Outcome:</strong></span> {{ $val($consultation, 'outcome') }}</td>
            </tr>
            <tr>
                <td colspan="4"><span class="label"><strong>Assessed by:</strong></span> {{ $val($consultation, 'assessed_by') }}</td>
            </tr>
        </table>
    </div>
